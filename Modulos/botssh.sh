...
[[ -z $1 || -z $2 || -z $3 ]] && {
    clear && echo "ERRO: INFORME O TOKEN, O LINK DO APP E O TEMPO DE EXPIRAÇÃO" && exit 1
}
[[ ! -e RESET ]] && touch RESET
...
criarteste() {
    [[ $(grep -wc ${callback_query_from_id} lista) != '0' ]] && {
      ShellBot.sendMessage --chat_id ${callback_query_message_chat_id} \
        --text "⚠️ VOCÊ JÁ CRIOU UM TESTE HOJE ⚠️"
      return 0
    }
    ...
    limite='1'
    tempo="$TEMPO_EXPIRA"
    tuserdate=$(date '+%C%y/%m/%d' -d " +$tempo hours")
...
enviarapp() {
    ShellBot.answerCallbackQuery --callback_query_id ${callback_query_id[$id]} \
        --text "♻️ BAIXANDO E ENVIANDO, AGUARDE..."

    local temp_apk_path="/tmp/app_$(date +%s).apk"

    wget -q -O "$temp_apk_path" "$LINK_APP"

    if [[ -s "$temp_apk_path" ]]; then
        ShellBot.sendDocument --chat_id ${callback_query_message_chat_id} \
            --document "@$temp_apk_path"
        rm "$temp_apk_path"
    else
        ShellBot.sendMessage --chat_id ${callback_query_message_chat_id} \
            --text "❌ Desculpe, não foi possível baixar o aplicativo. O link pode estar inválido."
    fi
    return 0
}
...
while :; do
  ...
  ShellBot.getUpdates --limit 100 --offset $(ShellBot.OffsetNext) --timeout 48
